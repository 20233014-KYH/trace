...
| AI 공급자 | **GPT-5.6 Luna (OpenAI) — 팀 결정 9/21.** Qwen 은 교수 추천(9/20)이었으나, 가격표상 OpenAI 최저가 440원/학기 < Qwen-Plus 650원/학기 라 가격 이점도 없었다. 리포트와 Side Chat 은 같은 모델을 쓴다. `core/llm.py` 어댑터라 Claude 로 바꿔 비교 가능. 키 없으면 fake |
