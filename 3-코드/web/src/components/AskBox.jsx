// 알약 위 질문칸 — 목업 13b/13c. 접힘: 알약과 같은 폭의 입력칸 하나. 펼침: 그 자리에서 위·왼쪽으로 자연스럽게 커져 채팅 패널. ✕ 로 다시 접힘.
// PC 앱에서는 이게 pywebview 알약 창 안에 들어간다 (항상 위 · 결정 11). 웹 뼈대에서는 화면 오른쪽 아래에 띄운다.
// 서버 POST /chat 한 곳만 부른다. 대화는 서버가 세션 맥락(kind:"chat")으로도 남긴다.
// 디자인 기준(흑연): 알약만 어두운 바탕 예외. Learn 파랑은 점·아이콘·보내기 버튼에만. 펼침 모션은 motion 의 layout 애니메이션 (350ms · 튐 없음).
import { useEffect, useRef, useState } from 'react';
import { AnimatePresence, motion } from 'motion/react';
import { ArrowUp, X } from 'lucide-react';

const W = 208;                                           // 알약 = 질문칸 폭 (px). 둘이 같아야 한 덩어리로 보인다
...
export default function AskBox({ onSend, open, setOpen, msgs, busy, error, seed, pill }) {
  ...
  return (
    <div className="fixed right-5 bottom-5 z-20 flex flex-col items-end gap-2">
      {/* 한 상자가 크기만 바뀐다 — 접힘(알약 폭 · 32px) ↔ 펼침(360px · 내용만큼). 오른쪽 아래 모서리가 고정점 */}
      <motion.div layout transition={SPRING} className="flex flex-col overflow-hidden"
                  style={{ width: open ? Math.min(360, window.innerWidth - 40) : W, borderRadius: 16, background: 'var(--surface)', boxShadow: SHADOW }}>
        <AnimatePresence mode="wait" initial={false}>
          {open ? (
            <motion.div key="panel" {...fade} className="flex flex-col">
              ...
              <div className="flex items-end gap-2 p-3" style={{ borderTop: '1px solid var(--line)' }}>
                ...
                <SendBtn className="size-8" onClick={send} disabled={busy || !q.trim()} />
              </div>
            </motion.div>
          ) : (
            <motion.button key="input" {...fade} onClick={() => setOpen(true)} className="flex h-8 w-full items-center gap-2 pl-3 pr-1 text-[12.5px] text-muted-foreground">
...
function SendBtn({ className = '', ...p }) {
  const Tag = p.onClick ? 'button' : 'span';
  return <Tag {...p} className={`flex shrink-0 items-center justify-center rounded-full text-white disabled:opacity-40 ${className}`} style={{ background: 'var(--learn)' }}><ArrowUp className="size-3.5" strokeWidth={2.5} /></Tag>;
}

/** 알약 — 목업 13. 질문칸과 같은 폭. 웹 뼈대에선 모양만 (시간·맥락 수). 실제 기록 제어는 수집기(PC 앱) 쪽 */
export function Pill({ elapsed, ctxCount }) {
  return (
    <div className="flex h-7 items-center gap-2 rounded-full px-3 text-[11.5px] text-white tabular-nums" style={{ width: W, background: 'rgba(24,24,27,.92)', boxShadow: '0 4px 14px rgba(0,0,0,.22)' }}>
      <i className="size-1.5 rounded-full" style={{ background: 'var(--learn)' }} /><b>{elapsed}</b><span className="text-white/60">맥락 {ctxCount}</span>
      <span className="ml-auto flex size-3.5 items-center justify-center rounded-full border border-white/50"><i className="block size-1.5 rounded-[1px] bg-white/80" /></span>
    </div>
  );
}
