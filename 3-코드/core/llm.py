...
팀 결정(9/21): **GPT-5.6 Luna** 를 쓴다. 가격표를 보니 OpenAI 최저가(440원/학기)가
Qwen-Plus(650원/학기)보다 싸서, 가격·한국어·키 발급 어느 쪽으로도 이유가 없었다.
어댑터는 그대로 둔다 — 같은 세션을 여러 공급자로 돌려 비교할 수 있다 (server/compare_llm.py).
리포트와 Side Chat 은 같은 모델을 쓴다. 채팅 내용이 맥락으로 남아 리포트에 들어가므로
모델을 나누면 한 모델이 쓴 답을 다른 모델이 요약하게 된다.
