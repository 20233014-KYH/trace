...
2026-09-20 · B 작성 → A와 1주차에 합의 후 `docs/api.md` 로 저장소에 · 이 문서가 확정되면 A는 수집기 없이 서버를, B는 서버 없이 수집기·화면을 만든다.

**9/20 합의 반영**: 해시 체인은 학생 PC와 서버 **양쪽에서** 만들고 서로 대조한다. 배치마다 체인 해시를 같이 보내고, 서버는 받은 이벤트로 재계산해 일치 여부와 수신 시각을 기록한다. Learn 맥락은 브라우저(Must) → VS Code(Should) → Word·PowerPoint(Should) 순서로 만들고, 시간이 부족하면 브라우저까지만. Learn에 교수 공개 없음.

원칙: **이벤트 한 줄(계약 ①)과 session 구조(계약 ②)는 그대로 쓴다.** API는 그 둘을 나르는 봉투일 뿐.
...
| AI 공급자 | **Qwen API (알리바바 Model Studio · 국제/싱가포르 엔드포인트) — 교수 추천 9/20, 최종 선택은 팀.** `core/llm.py` 어댑터라 Claude 로 바꿔 비교 가능. 키 없으면 fake |
